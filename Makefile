...
vcs/compile_rtl.sh: Bender.lock Bender.yml
	$(BENDER) script vcs -t rtl -t vcs -t simulation -t verilator -t relOBI -t uart_tmrg -t timer_unit_tmrg -DSYNTHESIS -DSIMULATION -DRELOBI -t relcore --vlog-arg="$(VCS_SCRIPT_ARGS)" --vlogan-bin="$(VLOGAN)" > $@
	chmod +x $@
