...
# see yosys/configuration.mk
if { [envVarValid "YOSYS_KEEP_HIER_INST"] } {
...
if { [envVarValid "YOSYS_BLACKBOX_MODULES"] } {
...
yosys proc
yosys tee -q -o "${report_dir}/${top_design}_initial.rpt" stat
yosys write_verilog -norename -noexpr -attr2comment ${build_dir}/${top_design}_yosys_initial.v
...
yosys fsm
yosys opt
yosys tee -q -o "${report_dir}/${top_design}_initial_opt.rpt" stat
yosys wreduce 
yosys peepopt
yosys opt_clean
yosys opt -full
...
yosys select -write ${report_dir}/${top_design}_registers.rpt t:*DFF*
set report [open ${report_dir}/${top_design}_instances.rpt "w"]
close $report
if { [envVarValid "YOSYS_REPORT_INSTS"] } {
    foreach sel $::env(YOSYS_REPORT_INSTS) {
        yosys tee -q -a ${report_dir}/${top_design}_instances.rpt  select -list {*}$sel
    }
}
...
# -----------------------------------------------------------------------------
# prep for openROAD
yosys write_verilog -norename -noexpr -attr2comment ${build_dir}/${top_design}_yosys_debug.v
...
yosys tee -q -o "${report_dir}/${top_design}_area_logic.rpt" stat -top $top_design {*}$tech_cells_args
