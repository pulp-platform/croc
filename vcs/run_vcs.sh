...
generate_netlist_openroad_flist() {
    if [ ! -f "../openroad/out/croc.v" ]; then
        run_cmd "echo [WARNING][Bender] Skipping OpenROAD netlist compile script: ../openroad/out/croc.v not found"
        run_cmd "rm -f compile_netlist_openroad.sh"
        return 0
    fi
