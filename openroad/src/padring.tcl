...
#   - pads start padKeepout microns away from the corner (pad -> bond -> gap)
#   - the remaining usable edge length is split into (numPads-1) equal gaps

set numPadsPerEdge 16
set padGap         20; # walk-off between IO pads and corners
set padKeepout     [expr {$padD + $padBond + $padGap}]
...
##########################################################################
set westSpan      $chipH
set westUsable    [expr {$westSpan - 2*$padKeepout - $padW}]
set westPitch     [expr {floor($westUsable / double($numPadsPerEdge - 1))}]
set westStart     [expr {$westSpan - $padKeepout - $padW}]
...
##########################################################################
set southSpan      $chipW
set southUsable    [expr {$southSpan - 2*$padKeepout - $padW}]
set southPitch     [expr {floor($southUsable / double($numPadsPerEdge - 1))}]
set southStart     $padKeepout
...
##########################################################################
set eastSpan      $chipH
set eastUsable    [expr {$eastSpan - 2*$padKeepout - $padW}]
set eastPitch     [expr {floor($eastUsable / double($numPadsPerEdge - 1))}]
set eastStart     $padKeepout
...
##########################################################################
set northSpan      $chipW
set northUsable    [expr {$northSpan - 2*$padKeepout - $padW}]
set northPitch     [expr {floor($northUsable / double($numPadsPerEdge - 1))}]
set northStart     [expr {$northSpan - $padKeepout - $padW}]
